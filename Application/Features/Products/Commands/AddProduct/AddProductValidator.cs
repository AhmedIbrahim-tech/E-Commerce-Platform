    ...
    public void ApplyCustomValidationRules()
    {
        RuleFor(c => c.Name)
            .MustAsync(async (name, cancellation) => !await _unitOfWork.Products.GetTableNoTracking()
                .AnyAsync(p => p.Name.Equals(name), cancellation))
            .WithMessage("Product with this name already exists");

        RuleFor(c => c.Slug)
            .MustAsync(async (slug, cancellation) => !await _unitOfWork.Products.GetTableNoTracking()
                .AnyAsync(p => p.Slug.Equals(slug), cancellation))
            .WithMessage("Product with this slug already exists");

        RuleFor(c => c.SKU)
            .MustAsync(async (sku, cancellation) => !await _unitOfWork.Products.GetTableNoTracking()
                .AnyAsync(p => p.SKU.Equals(sku), cancellation))
