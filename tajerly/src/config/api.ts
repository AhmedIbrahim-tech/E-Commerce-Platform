...
// Store is injected by Providers to avoid circular dependency: store → slices → services → api → store
type StoreInstance = {
    getState: () => { auth: { accessToken: string | null; refreshToken: string | null } };
...
const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://localhost:44322/api';
    ...
    async (error: AxiosError) => {
        ...
        try {
            ...
            if (result.succeeded && result.data) {
                ...
                const newRefreshToken = result.data.refreshToken.tokenString;
